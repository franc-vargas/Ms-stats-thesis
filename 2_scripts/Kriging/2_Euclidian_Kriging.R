...
info <- info |> dplyr::select(log_max.adultos_totales, max.adultos_totales, temperatura_promedio, salinidad_promedio, codigo)
...
vg <- GeoModels::GeoVariogram(max.summary.aysen@data$log_max.adultos_totales,
                        max.summary.aysen@coords,
                        maxdist = 1.3
)
plot(vg)
...
max(dist(max.summary.aysen@coords))
...
Ord.kriging <- GeoModels::GeoKrig(fit, data=info$log_max.adultos_totales, 
                                  ...
                                  distance = "Eucl",loc = d1)

Ord.prediction_data <- cbind(d1, exp(Ord.kriging$pred))
Ord.prediction_data <- as.data.frame(Ord.prediction_data)
...
Kriging.Mean.Plot <- 
  ...
  theme_bw()+
  labs(subtitle = "Aysén - Media constante - Eucl",
...
mean1 <- mean(log(info$temperatura_promedio))
...
Kriging.Cov.Plot <- 
  ...
  scale_colour_gradientn(colors = rev(rainbow(5)))+
  theme_bw()+
  labs(subtitle = "Aysén - Kriging con Covariables - Eucl",
