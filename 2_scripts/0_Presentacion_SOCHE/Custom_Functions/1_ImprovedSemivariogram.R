cs.variog <- function(response_variable, id_col, dist_object, 
  ...
  results$robust <- sqrt(abs(results$value))^4
...
plot.varioplot <- function(list, main = NULL, robust = F){
  ...
  if(robust != T){
    ...
    plot(I(round(semivariog,3)) ~ lag, data = list$plot.data, xlab = "Lag distance", ylab = "Semivariance",
         main = main)
  }else{
    if(is.null(main)){main <- "Cressie's Robust Semivariogram"}
    plot(I(round(robust,3)) ~ lag, data = list$plot.data, xlab = "Lag distance", ylab = "Semivariance",
         main = main)}
}
